...
def tv_loss(img, tv_weight):
    ...
    x_var = tf.reduce_sum(tf.square(img[:,1:,:,:] - img[:,:-1,:,:]))
    y_var = tf.reduce_sum(tf.square(img[:,:,1:,:] - img[:,:,:-1,:]))

    return tv_weight * (x_var + y_var)
...
def style_loss(feats, style_layers, style_targets, style_weights):
    ...
    for i, idx in enumerate(style_layers):
        G_g = gram_matrix(feats[idx])
        G_s = style_targets[i]
        w = style_weights[i]
        style_loss += w * tf.reduce_sum(tf.square(G_g - G_s))
...
def gram_matrix(features, normalize=True):
    ...
    gram = tf.einsum('bijc,bijd->cd', features, features)
...
def content_loss(content_weight, content_current, content_target):
    ...
    return content_weight * tf.reduce_sum(tf.square(content_current - content_target))

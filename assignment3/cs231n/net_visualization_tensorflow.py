...
def compute_saliency_maps(X, y, model):
    ...
    N, H, W, C = X.shape
    X_var = tf.Variable(X)
    indices = tf.constant(np.vstack((np.arange(N), y)).T.astype(int))

    with tf.GradientTape() as tape:
        # Shape (N, N_c)
        scores = model(X_var, training = False)
        S = tf.gather_nd(scores, indices)

    saliency = tape.gradient(S, X_var)
    saliency = tf.math.abs(saliency)
    saliency = tf.math.reduce_max(saliency, axis = -1).numpy()
...
def make_fooling_image(X, target_y, model):
    ...
    tgt_class = tf.constant(target_y, dtype = tf.int64)
    num_iters = 100
    ...
    for i in range(num_iters):
        with tf.GradientTape() as tape:
            ...
            tgt_score = scores[:, tgt_class]

        if tf.math.argmax(scores, axis = 1)[0] == tgt_class:
            print(f"The model is fooled at iteration-{i}")
            break

        # Shape (1, H, W, C)
        grad = tape.gradient(tgt_score, X_fooling_var)
        dX = learning_rate * grad / tf.norm(grad, ord = 2)
        X_fooling_var.assign_add(dX)

    # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    ##############################################################################
    #                             END OF YOUR CODE                               #
    ##############################################################################
    return X_fooling_var.numpy()
...
def class_visualization_update_step(X, model, target_y, l2_reg, learning_rate):
    ...
    with tf.GradientTape() as tape:
        ...
        tgt_score = scores[:, target_y] - l2_reg * tf.math.square(tf.norm(X, ord = 2))

    dX = tape.gradient(tgt_score, X)
    X += learning_rate * dX
